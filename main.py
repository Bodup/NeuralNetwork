...
def randomWeights(hiddenNeurons):
    global hiddenWeights
    global outputWeights
    hiddenWeights = 2 * np.random.random((hiddenNeurons, INPUT_NEURONS)) - 1
    outputWeights = 2 * np.random.random((OUTPUT_NEURONS, hiddenNeurons)) - 1
...
def backpropagate(inputData, targetData):
    ...
    # Weight adjustments
    outputWeights = outputWeights + LEARNING_RATE * np.dot(
        outputErrors * sigmoidPrime(output), hiddenOutput.T
    )
    hiddenWeights = hiddenWeights + LEARNING_RATE * np.dot(
        hiddenErrors * sigmoidPrime(hiddenOutput), inputData.T
    )
...
def analyze(configurations):
    ...
    for configuration in configurations:
        ...
        minutes = round(((end - start) / 60), 2)
        print(
            "training results for network with "
            + str(configuration)
            + " hidden neuron(s) saved. ("
            + str(minutes)
            + " minutes total, "
            + str(duration)
            + " on average)"
